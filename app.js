...
function setupAuthButtons() {
  ...
  loginBtn.addEventListener("click", async () => {
    ...
    const { error } = await client.auth.signInWithOtp({
      ...
      options: {
        emailRedirectTo:
          "https://amberl-tech.github.io/digital-wallet/",
      },
    });
...
/* ---------------------------------------------------------
   LISTEN FOR LOGIN AFTER MAGIC LINK
--------------------------------------------------------- */

function listenForAuthChanges() {
  client.auth.onAuthStateChange((event, session) => {
    if (session?.user) {
      wallet.profile.userId = session.user.id;
      updateAuthUI();
      loadBadges().then(renderBadges);
      renderProfile();
    }
  });
}
...
async function init() {
  setupAuthButtons();
  listenForAuthChanges(); // 🔥 required for magic link login
  await initUserId();
  updateAuthUI();
  await loadBadges();
  renderProfile();
